...
- **`App_Code/Repositories/`** (`MasterAntiqueRepairData`) — `TicketRepository`, `UserRepository`, `CommentRepository`, `AuditLogRepository`, `PasswordResetTokenRepository`. Each takes a `RepairShopContext` via constructor and exposes plain query/persist methods (`GetById`, `GetUnassigned`, `Add`, ...) — no business rules, no `SaveChanges()` calls of their own (the owning Service controls the save).
...
Despite `App_Code/IdentityModels.cs` (`ApplicationUser`/`ApplicationDbContext`/`UserManager`) still being present from the original VS template, **it is not the live auth path** — `Web.config`'s `<authentication mode="None" />` and the real login logic query `RepairShopContext.Users` directly and call `user.VerifyPassword(...)`. The only piece of that file still in use is `IdentityHelper.RedirectToReturnUrl` (open-redirect-safe `ReturnUrl` handling). Don't extend `ApplicationDbContext`/`UserManager` for new work — they're vestigial scaffold, not a second auth system in play.

The real mechanism:
- **`MasterAntiqueRepair/App_Code/AuthService.cs`** — owns the actual login/signup/password-reset business logic (throttle checks, lockout/soft-delete checks, password verification, audit logging, token issuance/validation). `Account/Login.aspx.cs`/`CustomerSignUp.aspx.cs`/`ForgotPassword.aspx.cs`/`ResetPassword.aspx.cs` are thin Controllers that call one `AuthService` method each and display whatever it throws. Lives in the website project (not `MasterAntiqueRepairData` like the other Services) because it needs OWIN sign-in and per-request IP — see the Services/Repositories section above.
- **`MasterAntiqueRepair/App_Code/RepairAuthHelper.cs`** — `SignIn(User, isPersistent)` builds a `ClaimsIdentity` (name, ID, and a role claim = the concrete domain type name, e.g. `"Employee"` — note `ObjectContext.GetObjectType` is used to unwrap EF6's lazy-loading proxy type first) and signs in via OWIN's cookie middleware. `RequireRole(Response, "RoleName")` guards protected pages (called directly by every page's `Page_Load`, not through `AuthService` — it's stateless cross-cutting plumbing, not a business flow). `GetCurrentUserId()` reads the ID claim back. Called internally by `AuthService` for the actual sign-in step.
...
- **`MasterAntiqueRepair/App_Code/IpThrottle.cs`** — in-memory, per-IP-and-scope rate limiter (20 attempts / 15 min sliding window), independent of `User`'s per-account lockout. Called from `AuthService` with scopes `"login"`/`"signup"`/`"forgotpassword"`. Process-local state, not DB-backed.
- **`Account/ForgotPassword.aspx` + `Account/ResetPassword.aspx`** — self-service password reset backed by `PasswordResetToken` (`MasterAntiqueRepairData/App_Code/`) via `AuthService`. No SMTP is configured anywhere in this app, so the reset link is shown directly on the `ForgotPassword` page rather than emailed — see `README.md`'s Security section for the full reasoning.
