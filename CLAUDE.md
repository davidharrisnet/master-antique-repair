...
- **`RepairShopContext`** (`"DefaultConnection"`) — the real app context: `DbSet<User> Users`, `DbSet<Ticket> Tickets`, `DbSet<Comment> Comments`, `DbSet<AuditLog> AuditLogs`. Has its own Migrations configuration under `Migrations/RepairShop/`, so EF6 uses `MigrateDatabaseToLatestVersion` as its initializer (not the plain `CreateDatabaseIfNotExists` default) — the first request against a DB with no matching `__MigrationHistory` row runs every migration starting with `InitialCreate`, building `dbo.Users`/`dbo.Tickets` from nothing.
- **`TestDbContext`** (`"TestConnection"`) — a separate scratch context (`TestItem`, `User`, `Ticket` `DbSet`s), its own database, its own Migrations configuration in `Migrations/` (the default location). Its only website-side consumer, `TestEF.aspx`, has been deleted — `TestDbContext`/`TestItem` are now unreferenced leftover code. Prefer `MasterAntiqueRepairScratch` for new experiments, not this.
