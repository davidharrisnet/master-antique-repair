...
**The ASP.NET Identity switch's migration (`AddAspNetIdentity`) has been generated and applied** — `User` now inherits `IdentityUser<int,...>`, and `RepairShopContext` gained the `Roles`/`UserRoles`/`UserClaims`/`UserLogins` tables. Existing accounts' passwords don't verify afterward, since Identity's password hash format differs from the old hand-rolled one — and it's not a clean login failure: the old format (`"{iterations}.{base64salt}.{base64hash}"`) isn't valid Base64, so Identity's `PasswordHasher.VerifyHashedPassword` throws an unhandled `FormatException` on first login attempt, surfacing as a 500 rather than "invalid password." `Seed-InitialUsers.ps1` alone won't fix this either — it skips any account that already exists by name, so a pre-existing `manager` row is left with its broken hash. The fix is `Reset-Database.ps1` followed by `Seed-InitialUsers.ps1` and/or `Seed-RegressionData.ps1` (see [Seeding initial accounts](#seeding-initial-accounts) and [Resetting the database](#resetting-the-database)) — every account has to be recreated under the new hash format, not just reset in place.
